...
}, error=function(e){}) ### skip if error and continue
}
...
##write output 
fwrite(pvalue,'/scratch/users/tcheandj/revision_phewas_plos_genetic/skat_test_results/results_skat_cont.$gene.txt',sep='\t')
" > skat.test.cont.$gene.R 


echo skat.test.cont.$gene.R 


 sbatch -p jpriest,normal,owners  --qos=normal -N 1 -n 1 -t 05:00:00 --mem=100000  -J $gene.skat -o log/$gene.skat.log -e log/$gene.skat.err  R CMD BATCH  skat.test.cont.$gene.R  
done
