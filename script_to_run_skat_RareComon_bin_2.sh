...
fwrite(pvalue,'/scratch/users/tcheandj/revision_phewas_plos_genetic/skat_test_results/results_skat_bin2.$gene.txtt',sep='\t')

" > skat.test.bin2.$gene.R 

echo skat.test.bin2.$gene.R 


 sbatch -p jpriest,normal,owners  --qos=normal -N 1 -n 1 -t 05:00:00 --mem=100000  -J $gene.skat -o log/$gene.skat.log -e log/$gene.skat.err  R CMD BATCH  skat.test.bin2.$gene.R   
done
